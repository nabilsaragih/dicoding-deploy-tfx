...
def model_builder(hp, vectorizer_layer):
    ...
    x = tf.keras.layers.Dense(hp.Choice('unit_1', [64, 128, 256]), activation="relu")(x)
    x = tf.keras.layers.Dropout(hp.Float('dropout_1', 0.1, 0.5, step=0.1))(x)
    x = tf.keras.layers.Dense(hp.Choice('unit_2', [32, 64, 128]), activation="relu")(x)
    x = tf.keras.layers.Dropout(hp.Float('dropout_2', 0.1, 0.5, step=0.1))(x)
